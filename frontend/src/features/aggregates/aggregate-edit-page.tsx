...
import { useAggregate, useUpdateAggregate } from "@/api/aggregates";
...
export default function AggregateEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: aggregate, isLoading } = useAggregate(id!);
  const update = useUpdateAggregate();
  ...
  if (!aggregate) return <ErrorState message="Aggregate not found" />;

  return (
    <>
      <PageHeader title="Edit Aggregate" actions={
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Back
        </button>
      } />
      <form onSubmit={handleSubmit((data) => update.mutate({ id: id!, data }, { onSuccess: () => navigate(`/aggregates/${id}`) }))} className="max-w-lg space-y-4">
        <div>

...
import { useCreateCause } from "@/api/causes";
...
export default function CauseCreatePage() {
  ...
  const onSubmit = (data: Form) => {
    create.mutate(data, {
      onSuccess: (res) => navigate(`/causes/${res.data.id}`),
    });
  };

  return (
    <>
      <PageHeader title="Create Cause" actions={
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          <button type="submit" disabled={create.isPending} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
            {create.isPending ? "Creating..." : "Create Cause"}
          </button>

...
import { useCreateAsset, useAssets } from "@/api/assets";

const schema = z.object({
  name: z.string().min(1, "Name is required").max(255),
  description: z.string().optional(),
  asset_type: z.string().min(1, "Type is required").max(100),
...
export default function AssetCreatePage() {
  ...
  return (
    <>
      <PageHeader title="Create Asset" actions={
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {errors.asset_type && <p className="mt-1 text-xs text-destructive">{errors.asset_type.message}</p>}
          </div>
          <div>
